package cool.spongecaptain.serial.producer;

import cool.spongecaptain.serial.dto.Cat;
import cool.spongecaptain.serial.serializer.CatSerializer;
...
public class KafkaProducerSample {
    public static void main(String[] args) {
        ...
        /**
          * 仅仅 value 是 Cat 类型
         */
        props.put("value.serializer", CatSerializer.class.getName());


        //构造具体类型为 org.apache.kafka.clients.producer.KafkaProducer 类型的生产者实例
        Producer<String, Cat> producer = new KafkaProducer<>(props);

        for(int i = 0; i < 10; i++) {
            //Producer#send 方法用于异步发送消息，这里的每一个消息都为键值对，并存放于 my-topic 主题下
            producer.send(new ProducerRecord<>("topic-of-codec", "SpongecaptainKey" + i, new Cat("cute "+i,i)));
        }
